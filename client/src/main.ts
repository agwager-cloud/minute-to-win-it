...
class MinuteApp{
  ...
  render(){
    ...
    if(this.room.phase==='lobby'){this.stopSpectating();this.hostMatchupsMode=false;this.managePlayersOpen=false;this.renderLobby();return;}
  ...
  renderGame(match:MatchState,spectator:boolean){
    ...
    if(match.status==='countdown'){
      appEl.innerHTML=this.shell(`<section class="game-shell"><div class="game-header"><div><span class="eyebrow">${spectator?'👁 LIVE SPECTATOR':champ?'👑 CHAMPIONSHIP MATCH':`COURT ${match.courtIndex+1}`}</span><h2>${game.symbol} ${esc(game.title)}</h2></div><div class="head-versus"><strong>${esc(a?.name)}</strong><span>VS</span><strong>${esc(b?.name)}</strong></div><div class="game-nav">${navButton}</div></div><div class="countdown-card"><small>NEXT MATCHUP STARTS</small><div id="countdown-number">3</div><h3>${esc(a?.name)} <span>VS</span> ${esc(b?.name)}</h3><p>${esc(game.tagline)}</p></div></section>`,'game-screen');this.bindCommon();document.querySelector('#back-matchups')?.addEventListener('click',()=>{this.stopSpectating();this.hostMatchupsMode=Boolean(this.me?.isHost);this.renderMatchups()});return;
    }
    const precision=match.precision;
    if(!precision){appEl.innerHTML=this.shell(`<section class="game-shell"><div class="game-header"><div><span class="eyebrow">${spectator?'👁 LIVE SPECTATOR':'MATCH'}</span><h2>${game.symbol} ${esc(game.title)}</h2></div><div class="game-nav">${navButton}</div></div><div class="loading-card">Preparing challenge…</div></section>`,'game-screen');this.bindCommon();document.querySelector('#back-matchups')?.addEventListener('click',()=>{this.stopSpectating();this.hostMatchupsMode=Boolean(this.me?.isHost);this.renderMatchups()});return;}
...
class PrecisionController{
  ...
  async runFuse(){
    const stage=this.stage();if(!stage)return;
    const V_W=1000,V_H=500,totalGems=10,maxScore=1000,mainSpeed=165,windowMs=135;
    ...
    const specs=[
      {name:'FUSE 1',subtitle:'WARM-UP WIRE',points:[{x:70,y:380},{x:190,y:130},{x:340,y:350},{x:500,y:110},{x:660,y:360},{x:820,y:140},{x:930,y:260}],fractions:[.14,.30,.47,.65,.82],costs:[24,27,29,26,23],branchLens:[74,68,82,70,64]},
      {name:'FUSE 2',subtitle:'RAPID FIRE',points:[{x:60,y:360},{x:175,y:160},{x:310,y:335},{x:430,y:130},{x:555,y:330},{x:655,y:170},{x:735,y:300},{x:805,y:165},{x:870,y:300},{x:940,y:210}],fractions:[.14,.34,.55,.73,.758],costs:[28,30,25,10,22],branchLens:[72,66,58,30,62]},
    ];
    stage.innerHTML=`<div class="fuse-game">
      <div class="fuse-topline"><div class="trial-label"><span id="fuse-stage-name">FUSE 1</span> · GEM <span id="fuse-gem-no">1</span> / ${totalGems}</div><div class="fuse-score">SCORE <strong id="fuse-score">0</strong><small>/ ${maxScore}</small></div></div>
      <div class="fuse-arena" id="fuse-arena"><canvas id="fuse-canvas" width="${V_W}" height="${V_H}"></canvas><div id="fuse-boom" class="fuse-boom"><span>💥</span><strong>BOOM!</strong></div><div class="fuse-stage-chip"><small id="fuse-stage-sub">WARM-UP WIRE</small><strong id="fuse-stage-count">1 / 2</strong></div></div>
      <div class="fuse-controls">
        <div class="fuse-status-card"><small>FOLLOW THE SPARK</small><strong id="fuse-status">GET READY</strong><em id="fuse-detail">Pinch only as the flame reaches a branch</em></div>
        <button id="fuse-pinch" class="fuse-pinch">PINCH!<small>Tap at a junction to steal its gem</small></button>
        <div class="fuse-meter"><div id="fuse-meter-fill"></div><span id="fuse-meter-label">FUSE LEFT 100%</span></div>
        <div class="fuse-risk"><strong>💎 RISK / REWARD</strong><span>Skip safely, or detour for points. Every gem burns extra fuse and makes the next timing tighter.</span></div>
        <div id="fuse-history" class="fuse-history">${Array.from({length:totalGems},(_,i)=>`<span data-fuse-history="${i}">${i+1}</span>`).join('')}</div>
      </div>
    </div>`;
    const canvas=document.querySelector<HTMLCanvasElement>('#fuse-canvas')!,ctx=canvas.getContext('2d')!,stageName=document.querySelector<HTMLElement>('#fuse-stage-name')!,stageSub=document.querySelector<HTMLElement>('#fuse-stage-sub')!,stageCount=document.querySelector<HTMLElement>('#fuse-stage-count')!,gemNo=document.querySelector<HTMLElement>('#fuse-gem-no')!,scoreEl=document.querySelector<HTMLElement>('#fuse-score')!,boom=document.querySelector<HTMLElement>('#fuse-boom')!,status=document.querySelector<HTMLElement>('#fuse-status')!,detail=document.querySelector<HTMLElement>('#fuse-detail')!,pinchBtn=document.querySelector<HTMLButtonElement>('#fuse-pinch')!,meterFill=document.querySelector<HTMLElement>('#fuse-meter-fill')!,meterLabel=document.querySelector<HTMLElement>('#fuse-meter-label')!;
    ...
    const updateHistory=()=>{historyEls.forEach((el,i)=>{const v=rounds[i];el.className=v<0?'':v===0?'skipped':v>=90?'perfect':v>=70?'good':'rough';el.textContent=v<0?String(i+1):v===0?`${i+1}: —`:`${i+1}: ${v}`;});scoreEl.textContent=String(totalScore());const unresolved=rounds.findIndex(v=>v<0);gemNo.textContent=String(unresolved<0?totalGems:unresolved+1);};
    ...
    for(let stageIndex=0;stageIndex<specs.length&&!this.destroyed;stageIndex++){
      const spec=specs[stageIndex],metric=makeMetric(spec.points),baseIndex=stageIndex*5,windowDist=mainSpeed*(windowMs/1000),junctions:Junction[]=[];
      for(let i=0;i<5;i++){const dist=metric.total*spec.fractions[i],p=at(metric,dist);const side=(i+stageIndex)%2===0?-1:1;const branchLen=spec.branchLens[i]+(seededUnit(this.state.seed,6140+stageIndex*20+i)-.5)*8;let tip={x:p.x+(-p.ty)*branchLen*side,y:p.y+p.tx*branchLen*side};tip={x:Math.max(38,Math.min(V_W-38,tip.x)),y:Math.max(42,Math.min(V_H-42,tip.y))};junctions.push({global:baseIndex+i,local:i,dist,point:{x:p.x,y:p.y},tip,branchLen:Math.hypot(tip.x-p.x,tip.y-p.y),fuseCost:spec.costs[i],status:'pending',score:0,errorMs:900});}
      stageName.textContent=spec.name;stageSub.textContent=spec.subtitle;stageCount.textContent=`${stageIndex+1} / ${specs.length}`;boom.className='fuse-boom';pinchBtn.disabled=true;pinchBtn.className='fuse-pinch';pinchBtn.innerHTML='GET READY<small>The spark starts automatically</small>';status.className='';status.textContent=stageIndex===0?'LIGHTING THE FUSE…':'FINAL FUSE…';detail.textContent=stageIndex===0?'Later junctions will arrive faster':'The last two branches are packed very close together';meterFill.style.width='100%';meterLabel.textContent='FUSE LEFT 100%';
      ...
      const markSkipped=(j:Junction,reason='SKIPPED GEM')=>{if(j.status!=='pending')return;j.status='skipped';j.score=0;j.errorMs=900;rounds[j.global]=0;errors[j.global]=900;historyEls[j.global]?.classList.add('skipped');updateHistory();lastResultText=reason;this.sendProgress(j.global+1,reason,totalScore());};
      const nextPending=()=>junctions.find(j=>j.status==='pending');
      const processPassed=()=>{for(const j of junctions){if(j.status==='pending'&&mainDistance>j.dist+windowDist)markSkipped(j,'GEM PASSED');}};
      const draw=()=>{
      ...
      onPinch=()=>{
        if(phase!=='main'||this.destroyed)return;const now=performance.now();if(now<pinchCooldownUntil)return;pinchCooldownUntil=now+280;
        // Advance the numeric distance to the exact pointer timestamp before
        // judging the pinch. This keeps the timing window independent of RAF
        // cadence: 60 Hz, 120 Hz and a momentarily slow frame all compare the
        // same distance-along-path value for the same real tap time.
        const pointerDt=Math.min(.045,Math.max(0,(now-lastFrame)/1000));if(pointerDt>0){mainDistance=Math.min(metric.total,mainDistance+mainSpeed*pointerDt);lastFrame=now;}processPassed();if(mainDistance>=metric.total){stageResolved=true;phase='done';return;}const j=nextPending();if(!j){status.className='warn';status.textContent='NO GEMS LEFT';detail.textContent='The bomb is all that remains';pinchBtn.classList.add('early');window.setTimeout(()=>pinchBtn.classList.remove('early'),180);return;}
        const diff=mainDistance-j.dist;if(diff<-windowDist){status.className='warn';status.textContent='TOO EARLY';detail.textContent='The spark has not reached that junction yet';pinchBtn.classList.add('early');window.setTimeout(()=>pinchBtn.classList.remove('early'),180);sound.beep(260,.04);return;}
        if(diff>windowDist){markSkipped(j,'GEM PASSED');status.className='bad';status.textContent='TOO LATE';detail.textContent='The spark already burned past that branch';sound.beep(180,.05);return;}
        const errorMs=Math.abs(diff)/mainSpeed*1000,ratio=Math.min(1,errorMs/windowMs),earned=Math.max(55,Math.min(100,Math.round(100-45*Math.pow(ratio,1.35))));j.status='collected';j.score=earned;j.errorMs=errorMs;rounds[j.global]=earned;errors[j.global]=Math.round(errorMs);active=j;branchPos=0;branchOut=true;mainDistance=j.dist;phase='branch';updateHistory();const label=earned>=97?'PERFECT PINCH!':earned>=88?'SHARP PINCH!':earned>=72?'CLEAN DETOUR!':'GEM CAUGHT!';status.className=earned>=88?'good':'okay';status.textContent=`${label} +${earned}`;detail.textContent=`Detour burns ${Math.max(1,Math.round(j.fuseCost/metric.total*100))}% extra fuse — be ready sooner`;pinchBtn.className='fuse-pinch hit';pinchBtn.innerHTML=`💎 +${earned}<small>${Math.round(errorMs)} ms from the junction centre</small>`;this.sendProgress(j.global+1,label,totalScore());sound.beep(earned>=95?1080:earned>=80?820:620,.07);
      };
      draw();await sleep(stageIndex===0?700:560);if(this.destroyed)return;phase='main';lastFrame=performance.now();pinchBtn.disabled=false;pinchBtn.className='fuse-pinch';pinchBtn.innerHTML='PINCH!<small>Tap exactly as the spark reaches a branch</small>';status.className='';status.textContent='FUSE BURNING';detail.textContent=stageIndex===0?'Skipping is safe — gems are optional':'Final fuse: watch for the rapid double junction';sound.beep(560,.06);
      await new Promise<void>(resolve=>{
        const frame=(now:number)=>{
          if(this.destroyed){resolve();return;}const dt=Math.min(.045,Math.max(0,(now-lastFrame)/1000));lastFrame=now;
          if(phase==='main'){mainDistance+=mainSpeed*dt;processPassed();if(mainDistance>=metric.total){mainDistance=metric.total;phase='done';stageResolved=true;for(const j of junctions)if(j.status==='pending')markSkipped(j,'BOMB REACHED');}}
          else if(phase==='branch'&&active){const branchSpeed=540;branchPos+=(branchOut?1:-1)*branchSpeed*dt;if(branchOut&&branchPos>=active.branchLen){branchPos=active.branchLen;branchOut=false;sound.beep(920,.045);}else if(!branchOut&&branchPos<=0){branchPos=0;const returned=active;mainDistance=Math.min(metric.total,returned.dist+returned.fuseCost);active=undefined;phase='main';pinchBtn.className='fuse-pinch';pinchBtn.innerHTML='PINCH!<small>Next junction may arrive sooner now</small>';processPassed();if(mainDistance>=metric.total){phase='done';stageResolved=true;for(const j of junctions)if(j.status==='pending')markSkipped(j,'BOMB REACHED');}}}
          const fusePct=Math.max(0,Math.min(100,(1-mainDistance/metric.total)*100));meterFill.style.width=`${fusePct}%`;meterLabel.textContent=`FUSE LEFT ${Math.ceil(fusePct)}%`;draw();
          if(stageResolved||phase==='done'){resolve();return;}this.raf=requestAnimationFrame(frame);
        };this.raf=requestAnimationFrame(frame);
      });
      if(this.raf)cancelAnimationFrame(this.raf);if(this.destroyed)return;onPinch=()=>{};pinchBtn.disabled=true;meterFill.style.width='0%';meterLabel.textContent='DETONATION';boom.className='fuse-boom show';status.className='bad';status.textContent=stageIndex===specs.length-1?'FINAL BOOM!':'BOOM — NEXT FUSE!';detail.textContent=lastResultText||`${junctions.filter(j=>j.status==='collected').length}/5 gems collected on this fuse`;pinchBtn.className='fuse-pinch boom';pinchBtn.innerHTML=`💥 BOOM!<small>${stageIndex===specs.length-1?'Run complete':'Loading the next fuse'}</small>`;sound.beep(115,.18);await sleep(stageIndex===specs.length-1?800:900);
    }
    if(this.destroyed)return;for(let i=0;i<rounds.length;i++)if(rounds[i]<0){rounds[i]=0;errors[i]=900;}const total=totalScore(),gems=rounds.filter(v=>v>0).length,collectedErrors=errors.filter((_,i)=>rounds[i]>0),avgError=collectedErrors.length?Math.round(collectedErrors.reduce((a,b)=>a+b,0)/collectedErrors.length):0,secondary=Math.min(20000,errors.reduce((a,b)=>a+b,0));
    status.className=total>=850?'good':total>=650?'okay':total>=350?'warn':'bad';status.textContent=total>=850?'FUSE MASTER!':total>=650?'GEM RUNNER!':total>=350?'GOOD HEIST':'PLAY IT BOLDER';detail.textContent=`${gems}/10 gems · ${gems?`${avgError} ms average pinch error`:'no successful pinches'}`;this.sendResult(total,secondary,`${total} / 1000 pts · ${gems}/10 gems${gems?` · ${avgError} ms avg pinch`:''}`,rounds);
  }
  async runTimeStop(){
