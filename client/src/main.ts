...
function precisionProgressText(gameId:string,value:number){if(gameId==='lights-out')return `${(value/1000).toFixed(3)} s`;if(gameId==='time-stop')return `${(value/1000).toFixed(2)} s error`;if(gameId==='blind-beat')return `${Math.round(value)} ms avg`;if(gameId==='shrink-ring'||gameId==='parry'||gameId==='overpour'||gameId==='charge-shot'||gameId==='stack'||gameId==='trace'||gameId==='ricochet')return `${Math.round(value)} pts`;return String(value);}
...
class PrecisionController{
  ...
  async runRicochet(){
    ...
    stage.innerHTML=`<div class="rico-game">
      <div class="rico-topline"><div class="trial-label">ONE SHOT <span>·</span> RICOCHET</div><div class="rico-score">SCORE <strong id="rico-score">—</strong><small>/ 1000</small></div></div>
      <div id="rico-arena" class="rico-arena"><canvas id="rico-canvas"></canvas></div>
      <div class="rico-controls">
        <div class="rico-status-card"><small id="rico-kicker">AIM THE BANK</small><strong id="rico-status">DRAG TO AIM</strong><em id="rico-detail">The shield blocks the direct route</em></div>
        <div class="rico-angle"><span>AIM</span><strong id="rico-angle">0°</strong><small>Only the first direction is shown</small></div>
        ...
        <div class="rico-tip">💥 Drag anywhere in the arena to aim · predict the bounce yourself</div>
      </div>
    </div>`;
    ...
    const start:P={x:.09,y:bankBottom?.24+seededUnit(this.state.seed,1601)*.13:.76-seededUnit(this.state.seed,1601)*.13};
    const target:P={x:.86,y:bankBottom?.22+seededUnit(this.state.seed,1602)*.16:.78-seededUnit(this.state.seed,1602)*.16};
    const bounceLine=bankBottom?1-ballRadius:ballRadius,mirrorY=2*bounceLine-target.y,idealDy=mirrorY-start.y,idealDx=target.x-start.x;
    ...
    const shieldEdge=bankBottom?.72+seededUnit(this.state.seed,1604)*.05:.28-seededUnit(this.state.seed,1604)*.05;
    let aimAngle=0,phase:'aiming'|'flying'|'done'='aiming',activePointer=-1,trail:P[]=[];
    let ball:P={...start},vx=0,vy=0,bounces=0,blocked=false,minDist=Infinity,flightMs=0;
    ...
    const updateAim=(e:PointerEvent)=>{if(phase!=='aiming')return;const r=arena.getBoundingClientRect(),nx=(e.clientX-r.left)/r.width,ny=(e.clientY-r.top)/r.height,dx=nx-start.x,dy=ny-start.y;if(dx<=.015)return;aimAngle=Math.max(-1.37,Math.min(1.37,Math.atan2(dy,dx)));angleEl.textContent=`${Math.round(Math.abs(aimAngle*180/Math.PI))}° ${aimAngle<0?'UP':'DOWN'}`;draw();};
    ...
    let resolveFire:(timedOut:boolean)=>void=()=>{};const firePromise=new Promise<boolean>(r=>resolveFire=r);const fire=(timedOut:boolean)=>{if(phase!=='aiming')return;phase='flying';activePointer=-1;resolveFire(timedOut);};fireBtn.addEventListener('pointerdown',e=>{e.preventDefault();fire(false)});
    const aimStarted=performance.now();const aimTimeout=window.setTimeout(()=>fire(true),12000);this.timers.push(aimTimeout);const aimTick=(now:number)=>{if(this.destroyed||phase!=='aiming')return;const elapsed=now-aimStarted,left=Math.max(0,12000-elapsed);timeFill.style.width=`${left/120}%`;timeLabel.textContent=`${(left/1000).toFixed(1)} s`;this.raf=requestAnimationFrame(aimTick)};this.raf=requestAnimationFrame(aimTick);
    ...
    const speed=.76;vx=Math.cos(aimAngle)*speed;vy=Math.sin(aimAngle)*speed;ball={...start};trail=[{...ball}];minDist=Infinity;bounces=0;blocked=false;const flightStart=performance.now();let last=flightStart;
