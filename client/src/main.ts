...
import { NetworkClient, type MatchState, type PlayerState, type PrecisionState, type RoomState, type SpectatorFrame } from './network';
...
function precisionProgressText(gameId:string,value:number){if(gameId==='lights-out')return `${(value/1000).toFixed(3)} s`;if(gameId==='time-stop')return `${(value/1000).toFixed(2)} s error`;if(gameId==='blind-beat')return `${Math.round(value)} ms avg`;if(gameId==='shrink-ring'||gameId==='parry'||gameId==='overpour'||gameId==='charge-shot'||gameId==='stack'||gameId==='trace'||gameId==='ricochet'||gameId==='knife-wheel'||gameId==='conveyor-chef'||gameId==='pole-balance'||gameId==='fuse')return `${Math.round(value)} pts`;return String(value);}

function safeSpectatorFragment(html:string){
  const template=document.createElement('template');template.innerHTML=html;
  template.content.querySelectorAll('script,iframe,object,embed,link,meta,style,img,a,audio,video,source,form').forEach(el=>el.remove());
  template.content.querySelectorAll<HTMLElement>('*').forEach(el=>{
    for(const attr of [...el.attributes]){
      ...
      if(name.startsWith('on')||name==='src'||name==='href'||name==='srcdoc'||name==='formaction'||name==='action'||(name==='style'&&(value.includes('url(')||value.includes('expression')||value.includes('javascript:'))))el.removeAttribute(attr.name);
    }
    if(el instanceof HTMLButtonElement||el instanceof HTMLInputElement){el.disabled=true;el.tabIndex=-1;}
  });
...
class MinuteApp{
  ...
  private presenceRepairAt=0; private emptyCourtsRepairAt=0; private serverClockOffset=0; private managePlayersOpen=false; private hostMatchupsMode=false; private spectatorRetryTimer?:number; private matchupScrollLeft=0; private matchupScrollInitialized=false; private matchupScrollLockUntil=0; private matchupDeferredRenderTimer?:number; private spectatorFrameReceivedAt=new Map<string,number>();
  ...
  private frameKey(matchId:string,playerId:string){return `${matchId}::${playerId}`}
  ...
  private onSpectatorFrame(frame:SpectatorFrame){if(!frame?.matchId||!frame.playerId)return;const key=this.frameKey(frame.matchId,frame.playerId);this.spectatorFrames.set(key,frame);this.spectatorFrameReceivedAt.set(key,Date.now());if(frame.matchId===this.spectatingMatchId&&frame.playerId===this.spectatingPlayerId){document.querySelector('#spectator-stale-badge')?.remove();this.applySpectatorFrame(frame)}}
  ...
  private ensureSpectatorRetry(matchId:string,povPlayerId:string){this.clearSpectatorRetry();const key=this.frameKey(matchId,povPlayerId);const check=()=>{if(this.spectatingMatchId!==matchId||this.spectatingPlayerId!==povPlayerId){this.clearSpectatorRetry();return;}const own=this.activeMatchFor(this.session?.playerId);if(own){this.clearSpectatorRetry();this.stopSpectating();this.render();return;}const last=this.spectatorFrameReceivedAt.get(key)||0;if(last&&Date.now()-last<1800)return;this.net.send({type:'spectate-match',matchId,povPlayerId});const root=document.querySelector<HTMLElement>('#spectator-live-root');const progress=this.findMatch(matchId)?.precision?.progress[povPlayerId];const label=progress?.label?esc(progress.label):'LIVE GAME IN PROGRESS';if(root){if(root.querySelector('.spectator-feed-loading'))root.innerHTML=`<div class="spectator-feed-loading"><div class="spinner"></div><strong>CONNECTING TO LIVE PLAYER VIEW…</strong><small>${label} · Live view is retrying automatically.</small></div>`;else if(!root.querySelector('#spectator-stale-badge'))root.insertAdjacentHTML('beforeend','<div id="spectator-stale-badge" class="spectator-stale-badge">LIVE VIEW RECONNECTING…</div>');}};check();this.spectatorRetryTimer=window.setInterval(check,1200);}
  ...
  onRoomState(r:RoomState){
    const sampleOffset=r.serverTime-Date.now();this.serverClockOffset=this.serverClockOffset?this.serverClockOffset*.75+sampleOffset*.25:sampleOffset;
    this.room=r;
    ...
    if(me?.isHost&&r.phase!=='lobby'&&r.courts.length===0&&(r.hostOptedIn!==false||r.players.some(p=>!p.isHost&&!p.isBot&&p.connected))&&this.net.isOnline()&&Date.now()-this.emptyCourtsRepairAt>2500){
    ...
    // Keep the live spectator DOM mounted while the watched court is running.
    // Progress broadcasts can arrive several times during a mini-game; rebuilding
    // the entire spectator scene for each one used to replace the live stream with
    // CONNECTING... over and over. The streamed player view now stays continuous.
    const ownNow=this.activeMatchFor(this.session?.playerId);const watchedNow=this.findMatch(this.spectatingMatchId);const spectatorRoot=document.querySelector<HTMLElement>('#spectator-live-root');
    if(watchedNow&&!ownNow&&watchedNow.status==='playing'&&spectatorRoot){const humans=watchedNow.playerIds.filter(id=>!this.player(id)?.isBot);const current=this.spectatingPlayerId;if(current&&watchedNow.precision?.results[current]){const next=humans.find(id=>!watchedNow.precision?.results[id]);if(next&&next!==current){this.switchSpectatorPov(next);return;}}return;}
    if(this.controller){this.controller.destroy();this.controller=undefined;}
    if(this.spectatingMatchId&&!this.findMatch(this.spectatingMatchId)){this.net.send({type:'stop-spectating'});this.spectatingMatchId=undefined;this.spectatingPlayerId=undefined;}
    const existingScroller=document.querySelector<HTMLElement>('.courts-scroll');if(existingScroller){this.matchupScrollLeft=existingScroller.scrollLeft;this.matchupScrollInitialized=true;}
    if(document.querySelector('.matchup-screen')&&Date.now()<this.matchupScrollLockUntil){if(this.matchupDeferredRenderTimer)clearTimeout(this.matchupDeferredRenderTimer);this.matchupDeferredRenderTimer=window.setTimeout(()=>{this.matchupDeferredRenderTimer=undefined;this.render();},Math.max(40,this.matchupScrollLockUntil-Date.now()+20));return;}
    this.render();
  }
  render(){
    const old=this.controller;if(old){old.destroy();this.controller=undefined;}
    if(!this.room||!this.session){this.renderStart();return;}
    if(this.room.phase==='lobby'){this.stopSpectating();this.hostMatchupsMode=false;this.managePlayersOpen=false;this.matchupScrollLeft=0;this.matchupScrollInitialized=false;this.renderLobby();return;}
  ...
  renderMatchups(){
    const existingScroller=document.querySelector<HTMLElement>('.courts-scroll');if(existingScroller){this.matchupScrollLeft=existingScroller.scrollLeft;this.matchupScrollInitialized=true;}
    const room=this.room!,game=this.selectedGame;const hostOptedIn=room.hostOptedIn!==false;const champion=room.currentChampionId;const ranked=[...room.players].filter(p=>!p.isBot).sort((a,b)=>a.id===champion?-1:b.id===champion?1:b.points-a.points||a.name.localeCompare(b.name));
    const waitingSet=new Set(room.courts.flatMap(c=>c.waiting));const lateSet=new Set(room.lateJoinQueue);
    const courts=room.courts.map((c,i)=>{const m=c.activeMatch;const a=this.player(m?.playerIds[0]),b=this.player(m?.playerIds[1]);const status=m?.status||'waiting';const queued=c.waiting.map(id=>this.player(id)).filter(Boolean) as PlayerState[];const waitingLine=queued.length?`<div class="court-waiting"><small>NEXT / WAITING</small><span>${queued.map(p=>esc(p.name)).join(' · ')}</span></div>`:'';return `<button class="court-card ${i===room.courts.length-1?'championship':''} ${m&&m.status!=='ready'?'clickable':''}" ${m&&m.status!=='ready'?`data-watch="${m.id}"`:''}><div class="court-head"><strong>${i===room.courts.length-1?'👑 CHAMPIONSHIP':i===0?'LOWEST COURT':`COURT ${i+1}`}</strong><span class="status ${status}">${status.toUpperCase()}</span></div>${m?`<div class="versus"><div><span class="presence ${a?.connected?'on':''}"></span><strong>${esc(a?.name||'Player')}</strong><small>${a?.isBot?'BOT':`${a?.points||0} pts`}</small></div><b>VS</b><div><span class="presence ${b?.connected?'on':''}"></span><strong>${esc(b?.name||'Player')}</strong><small>${b?.isBot?'BOT':`${b?.points||0} pts`}</small></div></div>${waitingLine}`:`<div class="empty-court">${waitingLine||'Waiting for two players'}</div>`}</button>`}).join('');
    ...
    document.querySelector('#host-participation')?.addEventListener('click',()=>{const optingOut=hostOptedIn;const mine=this.activeMatchFor(this.session?.playerId);if(optingOut&&mine&&!window.confirm(`Opt out now? ${this.player(mine.playerIds.find(id=>id!==this.session?.playerId))?.name||'Your opponent'} will receive the win for this match.`))return;this.net.send({type:'set-host-participation',participating:!hostOptedIn});});
    document.querySelector('#manage-players')?.addEventListener('click',()=>{this.managePlayersOpen=true;this.renderMatchups()});document.querySelector('#close-manage')?.addEventListener('click',()=>{this.managePlayersOpen=false;this.renderMatchups()});document.querySelector('#manage-backdrop')?.addEventListener('click',e=>{if(e.target===e.currentTarget){this.managePlayersOpen=false;this.renderMatchups()}});document.querySelectorAll<HTMLElement>('[data-manage-kick]').forEach(el=>el.addEventListener('click',()=>{const pl=this.player(el.dataset.manageKick);if(!pl)return;if(window.confirm(`Remove ${pl.name} from the room? They must use a different name to rejoin.`))this.net.send({type:'kick-player',playerId:pl.id})}));document.querySelectorAll<HTMLElement>('[data-watch]').forEach(el=>el.addEventListener('click',()=>this.beginSpectating(String(el.dataset.watch||''))));
    const scroller=document.querySelector<HTMLElement>('.courts-scroll');if(scroller){const restore=()=>{const max=Math.max(0,scroller.scrollWidth-scroller.clientWidth);if(this.me?.isHost&&!this.matchupScrollInitialized){scroller.scrollLeft=max;this.matchupScrollLeft=max;this.matchupScrollInitialized=true;}else{scroller.scrollLeft=Math.max(0,Math.min(max,this.matchupScrollLeft));if(!this.matchupScrollInitialized)this.matchupScrollInitialized=true;}};requestAnimationFrame(()=>requestAnimationFrame(restore));const protect=()=>{this.matchupScrollLeft=scroller.scrollLeft;this.matchupScrollInitialized=true;this.matchupScrollLockUntil=Date.now()+420;};scroller.addEventListener('scroll',protect,{passive:true});scroller.addEventListener('pointerdown',()=>{this.matchupScrollLockUntil=Date.now()+700;},{passive:true});scroller.addEventListener('touchstart',()=>{this.matchupScrollLockUntil=Date.now()+700;},{passive:true});}
  }
  renderGame(match:MatchState,spectator:boolean){
    const game=GAME_BY_ID.get(match.precision?.gameId||this.room!.selectedGameId)||this.selectedGame;const a=this.player(match.playerIds[0]),b=this.player(match.playerIds[1]);const champ=match.courtIndex===this.room!.courts.length-1;const hostCanNavigate=Boolean(this.me?.isHost);const navButton=spectator||hostCanNavigate?'<button id="back-matchups" class="secondary small">← MATCHUPS</button>':'';
    if(match.status==='countdown'){
      appEl.innerHTML=this.shell(`<section class="game-shell"><div class="game-header"><div><span class="eyebrow">${spectator?'👁 LIVE SPECTATOR':champ?'👑 CHAMPIONSHIP MATCH':`COURT ${match.courtIndex+1}`}</span><h2>${game.symbol} ${esc(game.title)}</h2></div><div class="head-versus"><strong>${esc(a?.name)}</strong><span>VS</span><strong>${esc(b?.name)}</strong></div><div class="game-nav">${navButton}</div></div><div class="countdown-card"><small>NEXT MATCHUP STARTS</small><div id="countdown-number">3</div><h3>${esc(a?.name)} <span>VS</span> ${esc(b?.name)}</h3><p>${esc(game.tagline)}</p></div></section>`,'game-screen');this.bindCommon();document.querySelector('#back-matchups')?.addEventListener('click',()=>{this.stopSpectating();this.hostMatchupsMode=Boolean(this.me?.isHost);if(this.me?.isHost)this.matchupScrollInitialized=false;this.renderMatchups()});return;
    }
    const precision=match.precision;
    if(!precision){appEl.innerHTML=this.shell(`<section class="game-shell"><div class="game-header"><div><span class="eyebrow">${spectator?'👁 LIVE SPECTATOR':'MATCH'}</span><h2>${game.symbol} ${esc(game.title)}</h2></div><div class="game-nav">${navButton}</div></div><div class="loading-card">Preparing challenge…</div></section>`,'game-screen');this.bindCommon();document.querySelector('#back-matchups')?.addEventListener('click',()=>{this.stopSpectating();this.hostMatchupsMode=Boolean(this.me?.isHost);if(this.me?.isHost)this.matchupScrollInitialized=false;this.renderMatchups()});return;}
    if(precision.phase==='results'){
      ...
      appEl.innerHTML=this.shell(`<section class="game-shell"><div class="game-header result-nav-header"><div><span class="eyebrow">${spectator?'👁 LIVE SPECTATOR':'MATCH COMPLETE'}</span><h2>${game.symbol} ${esc(game.title)}</h2></div><div class="game-nav">${navButton}</div></div><div class="result-card"><span class="eyebrow">${game.title.toUpperCase()} RESULT</span><div class="trophy">🏆</div><h2>${esc(winner?.name)} WINS</h2><div class="result-versus"><div><strong>${esc(a?.name)}</strong><span>${esc(ra?.display||'—')}</span></div><b>VS</b><div><strong>${esc(b?.name)}</strong><span>${esc(rb?.display||'—')}</span></div></div><div class="move-right">+1 MATCH POINT · WINNER MOVES RIGHT →</div></div></section>`,'game-screen');this.bindCommon();document.querySelector('#back-matchups')?.addEventListener('click',()=>{this.stopSpectating();this.hostMatchupsMode=Boolean(this.me?.isHost);if(this.me?.isHost)this.matchupScrollInitialized=false;this.renderMatchups()});sound.beep(760,.12);return;
    }
    if(spectator){
      const humans=match.playerIds.filter(id=>!this.player(id)?.isBot);let pov=this.spectatingPlayerId;if(!pov||!humans.includes(pov)||(precision.results[pov]&&humans.some(id=>!precision.results[id])))pov=humans.find(id=>!precision.results[id])||humans[0];this.spectatingPlayerId=pov;
      ...
      this.bindCommon();document.querySelector('#back-matchups')?.addEventListener('click',()=>{this.stopSpectating();this.hostMatchupsMode=Boolean(this.me?.isHost);if(this.me?.isHost)this.matchupScrollInitialized=false;this.renderMatchups()});document.querySelectorAll<HTMLElement>('[data-pov]').forEach(el=>el.addEventListener('click',()=>this.switchSpectatorPov(String(el.dataset.pov||''))));const cached=pov?this.spectatorFrames.get(this.frameKey(match.id,pov)):undefined;if(cached)this.applySpectatorFrame(cached);if(pov)this.ensureSpectatorRetry(match.id,pov);return;
    }
    ...
    this.bindCommon();document.querySelector('#back-matchups')?.addEventListener('click',()=>{this.hostMatchupsMode=true;this.matchupScrollInitialized=false;this.renderMatchups()});const myId=this.session!.playerId;const isParticipant=match.playerIds.includes(myId);if(isParticipant&&!precision.results[myId]){this.controller=new PrecisionController(this,match,precision,game);this.controller.start();}else this.renderPrecisionWatcher(match,precision,game,false);
  }
  ...
  tick(){
    const num=document.querySelector<HTMLElement>('#countdown-number');if(num&&this.room){const m=this.findMatch(this.spectatingMatchId)||this.activeMatchFor(this.session?.playerId);if(m?.startsAt){const n=Math.max(1,Math.ceil((m.startsAt-this.serverNow())/1000));num.textContent=String(n)}}
    const pause=this.room&&(this.findMatch(this.spectatingMatchId)?.disconnectPause||this.activeMatchFor(this.session?.playerId)?.disconnectPause);if(pause){const pl=this.player(pause.playerId);this.showPersistentNotice(`${pl?.name||'Player'} disconnected — waiting up to ${Math.max(0,Math.ceil((pause.graceUntil-this.serverNow())/1000))}s to reconnect`)}else this.clearPersistentNotice();
  }
...
class PrecisionController{
  ...
  async runFuse(){
    const stage=this.stage();if(!stage)return;
    const V_W=1000,V_H=500,totalGems=10,maxScore=1000;
    const rounds=Array.from({length:totalGems},()=>-1),errors=Array.from({length:totalGems},()=>900);
    type Pt={x:number;y:number};
    type MetricSeg={a:Pt;b:Pt;start:number;len:number;dx:number;dy:number};
    type Metric={points:Pt[];segments:MetricSeg[];total:number};
    type Junction={global:number;local:number;dist:number;point:Pt;tip:Pt;branchLen:number;fuseCost:number;status:'pending'|'collected'|'skipped';score:number;errorMs:number};
    type FuseSpec={name:string;subtitle:string;speed:number;windowMs:number;branchSpeed:number;points:Pt[];gaps:number[];costs:number[];branchLens:number[]};
    const clamp=(v:number,a:number,b:number)=>Math.max(a,Math.min(b,v));
    const fuseUnit=(salt:number)=>{let x=(this.state.seed+Math.imul(salt+1,0x9e3779b9))>>>0;x=Math.imul(x^(x>>>16),0x21f0aaad);x=Math.imul(x^(x>>>15),0x735a2d97);x^=x>>>15;return(x>>>0)/0x100000000;};
    const makePath=(stageIndex:number):Pt[]=>{
      const warmPatterns=[
        [.78,.20,.72,.30,.82,.24,.66,.44],
        [.60,.16,.42,.82,.28,.70,.20,.56],
        [.82,.50,.18,.68,.26,.78,.40,.24],
      ...
      return pattern.map((v,i)=>{
        ...
        return{x:clamp(x,44,V_W-44),y:clamp(y,64,V_H-62)};
      });
    };
    ...
    stage.innerHTML=`<div class="fuse-game">
      ...
      <div class="fuse-arena" id="fuse-arena"><canvas id="fuse-canvas" width="${V_W}" height="${V_H}"></canvas><div id="fuse-boom" class="fuse-boom"><span>💥</span><strong>BOOM!</strong></div><div class="fuse-stage-chip"><small id="fuse-stage-sub">GEM RUN</small><strong id="fuse-stage-count">1 / 2</strong></div></div>
      <div class="fuse-controls">
        <div class="fuse-status-card"><small>GOAL: STEAL THE GEMS</small><strong id="fuse-status">THE BOMB ALWAYS BLOWS</strong><em id="fuse-detail">Pinch orange junctions as the spark reaches them</em></div>
        <button id="fuse-pinch" class="fuse-pinch">PINCH!<small>Tap as the spark reaches an orange junction</small></button>
        <div class="fuse-meter"><div id="fuse-meter-fill"></div><span id="fuse-meter-label">FUSE TO BOMB 100%</span></div>
        <div class="fuse-risk"><strong>🎯 OBJECTIVE</strong><span>Collect as many of the 10 gems as you can. Skipping is safe but scores 0. Every gem shortens the remaining fuse, so the next junction comes sooner.</span></div>
        <div id="fuse-history" class="fuse-history">${Array.from({length:totalGems},(_,i)=>`<span data-fuse-history="${i}">${i+1}</span>`).join('')}</div>
      </div>
    </div>`;
    const canvas=document.querySelector<HTMLCanvasElement>('#fuse-canvas')!,ctx=canvas.getContext('2d')!,stageName=document.querySelector<HTMLElement>('#fuse-stage-name')!,stageSub=document.querySelector<HTMLElement>('#fuse-stage-sub')!,stageCount=document.querySelector<HTMLElement>('#fuse-stage-count')!,targetNo=document.querySelector<HTMLElement>('#fuse-target-no')!,gemsEl=document.querySelector<HTMLElement>('#fuse-gems')!,scoreEl=document.querySelector<HTMLElement>('#fuse-score')!,boom=document.querySelector<HTMLElement>('#fuse-boom')!,status=document.querySelector<HTMLElement>('#fuse-status')!,detail=document.querySelector<HTMLElement>('#fuse-detail')!,pinchBtn=document.querySelector<HTMLButtonElement>('#fuse-pinch')!,meterFill=document.querySelector<HTMLElement>('#fuse-meter-fill')!,meterLabel=document.querySelector<HTMLElement>('#fuse-meter-label')!;
    const historyEls=Array.from(document.querySelectorAll<HTMLElement>('[data-fuse-history]'));
    const makeMetric=(points:Pt[]):Metric=>{let start=0;const segments:MetricSeg[]=[];for(let i=0;i<points.length-1;i++){const a=points[i],b=points[i+1],dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy);segments.push({a,b,start,len,dx,dy});start+=len;}return{points,segments,total:start};};
    const at=(metric:Metric,d:number)=>{const clamped=Math.max(0,Math.min(metric.total,d));let seg=metric.segments[metric.segments.length-1];for(const s of metric.segments){if(clamped<=s.start+s.len){seg=s;break;}}const t=seg.len?Math.max(0,Math.min(1,(clamped-seg.start)/seg.len)):0;return{x:seg.a.x+seg.dx*t,y:seg.a.y+seg.dy*t,tx:seg.len?seg.dx/seg.len:1,ty:seg.len?seg.dy/seg.len:0};};
    const drawPath=(metric:Metric,limit:number,stroke:string,width:number,glow=0)=>{ctx.save();ctx.lineCap='round';ctx.lineJoin='round';ctx.strokeStyle=stroke;ctx.lineWidth=width;if(glow){ctx.shadowColor=stroke;ctx.shadowBlur=glow;}ctx.beginPath();ctx.moveTo(metric.points[0].x,metric.points[0].y);for(const seg of metric.segments){if(limit<=seg.start)break;if(limit>=seg.start+seg.len){ctx.lineTo(seg.b.x,seg.b.y);continue;}const t=(limit-seg.start)/seg.len;ctx.lineTo(seg.a.x+seg.dx*t,seg.a.y+seg.dy*t);break;}ctx.stroke();ctx.restore();};
    const totalScore=()=>rounds.reduce((sum,v)=>sum+Math.max(0,v),0);
    const gemCount=()=>rounds.filter(v=>v>0).length;
    const updateHistory=()=>{historyEls.forEach((el,i)=>{const v=rounds[i];el.className=v<0?'':v===0?'skipped':v>=90?'perfect':v>=70?'good':'rough';el.textContent=v<0?String(i+1):v===0?`${i+1}: —`:`${i+1}: ${v}`;});scoreEl.textContent=String(totalScore());gemsEl.textContent=String(gemCount());};
    let onPinch:()=>void=()=>{};pinchBtn.addEventListener('pointerdown',e=>{e.preventDefault();onPinch();});
    updateHistory();await sleep(520);if(this.destroyed)return;
    for(let stageIndex=0;stageIndex<specs.length&&!this.destroyed;stageIndex++){
      ...
      const processPassed=()=>{for(const j of junctions){if(j.status==='pending'&&mainDistance>j.dist+windowDist)markSkipped(j,'GEM MISSED');}};
      const draw=()=>{
        ctx.clearRect(0,0,V_W,V_H);const bg=ctx.createLinearGradient(0,0,V_W,V_H);bg.addColorStop(0,'#090817');bg.addColorStop(.55,'#150b25');bg.addColorStop(1,'#210810');ctx.fillStyle=bg;ctx.fillRect(0,0,V_W,V_H);
        ctx.save();ctx.globalAlpha=.18;ctx.strokeStyle='#ff405d';ctx.lineWidth=1;for(let x=25;x<V_W;x+=50){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,V_H);ctx.stroke();}for(let y=25;y<V_H;y+=50){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(V_W,y);ctx.stroke();}ctx.restore();
        drawPath(metric,metric.total,'#2b1a1c',24);drawPath(metric,metric.total,'#9a5c32',12);ctx.save();ctx.setLineDash([4,9]);drawPath(metric,metric.total,'rgba(255,220,144,.42)',3);ctx.restore();drawPath(metric,mainDistance,'#ff5a35',14,18);drawPath(metric,mainDistance,'#ffd25b',5,12);
        for(const j of junctions){
          ctx.save();ctx.lineCap='round';ctx.strokeStyle=j.status==='skipped'?'rgba(112,81,90,.45)':'#6b3d2d';ctx.lineWidth=14;ctx.beginPath();ctx.moveTo(j.point.x,j.point.y);ctx.lineTo(j.tip.x,j.tip.y);ctx.stroke();ctx.strokeStyle=j.status==='skipped'?'rgba(133,106,112,.3)':'#bb7240';ctx.lineWidth=7;ctx.stroke();ctx.setLineDash([3,8]);ctx.strokeStyle='rgba(255,229,158,.4)';ctx.lineWidth=2;ctx.stroke();ctx.setLineDash([]);
          const approach=phase==='main'&&j.status==='pending'?clamp(1-Math.abs(mainDistance-j.dist)/(spec.speed*.52),0,1):0;if(approach>0){ctx.beginPath();ctx.arc(j.point.x,j.point.y,15+approach*9,0,Math.PI*2);ctx.strokeStyle=`rgba(255,210,82,${(.15+approach*.7).toFixed(2)})`;ctx.lineWidth=3+approach*3;ctx.shadowColor='#ffd34f';ctx.shadowBlur=18*approach;ctx.stroke();ctx.shadowBlur=0;}
          ctx.beginPath();ctx.arc(j.point.x,j.point.y,10,0,Math.PI*2);ctx.fillStyle=j.status==='pending'?'#22101a':j.status==='collected'?'#3b2b0b':'#1b1520';ctx.fill();ctx.strokeStyle=j.status==='pending'?'#ff9b5c':j.status==='collected'?'#ffd85a':'#66505a';ctx.lineWidth=3;ctx.stroke();
          if(j.status==='pending'||(active===j&&phase==='branch'&&branchOut)){ctx.font='31px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.shadowColor='#65f4ff';ctx.shadowBlur=13;ctx.fillText('💎',j.tip.x,j.tip.y);}else if(j.status==='collected'){ctx.font='20px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('✨',j.tip.x,j.tip.y);}ctx.restore();
        }
        if(active&&phase==='branch'){ctx.save();ctx.lineCap='round';ctx.strokeStyle='#ff6b35';ctx.lineWidth=12;ctx.shadowColor='#ff6b35';ctx.shadowBlur=14;ctx.beginPath();ctx.moveTo(active.point.x,active.point.y);const t=clamp(branchPos/Math.max(1,active.branchLen),0,1),ex=active.point.x+(active.tip.x-active.point.x)*t,ey=active.point.y+(active.tip.y-active.point.y)*t;ctx.lineTo(ex,ey);ctx.stroke();ctx.restore();}
        const spark=active&&phase==='branch'?(()=>{const t=clamp(branchPos/Math.max(1,active.branchLen),0,1);return{x:active.point.x+(active.tip.x-active.point.x)*t,y:active.point.y+(active.tip.y-active.point.y)*t};})():at(metric,mainDistance);ctx.save();ctx.beginPath();ctx.arc(spark.x,spark.y,9,0,Math.PI*2);ctx.fillStyle='#fff5b3';ctx.shadowColor='#ff6535';ctx.shadowBlur=28;ctx.fill();ctx.beginPath();ctx.arc(spark.x,spark.y,4.5,0,Math.PI*2);ctx.fillStyle='#ffffff';ctx.fill();for(let s=0;s<5;s++){const a=(performance.now()/110+s*1.7)%(Math.PI*2),r=15+s*2;ctx.beginPath();ctx.arc(spark.x-Math.cos(a)*r,spark.y-Math.sin(a)*r,1.8,0,Math.PI*2);ctx.fillStyle=s%2?'#ffcf55':'#ff5939';ctx.fill();}ctx.restore();
        const end=metric.points[metric.points.length-1];ctx.save();ctx.font='64px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.shadowColor='#ff345d';ctx.shadowBlur=20;ctx.fillText('💣',end.x,end.y);ctx.restore();
      };
      onPinch=()=>{
        if(phase!=='main'||this.destroyed)return;const now=performance.now();if(now<pinchCooldownUntil)return;pinchCooldownUntil=now+220;
        const pointerDt=Math.min(.045,Math.max(0,(now-lastFrame)/1000));if(pointerDt>0){mainDistance=Math.min(metric.total,mainDistance+spec.speed*pointerDt);lastFrame=now;}processPassed();if(mainDistance>=metric.total){stageResolved=true;phase='done';return;}const j=nextPending();if(!j){status.className='warn';status.textContent='ALL JUNCTIONS PASSED';detail.textContent='The bomb is the only thing left on this fuse';pinchBtn.classList.add('early');window.setTimeout(()=>pinchBtn.classList.remove('early'),180);return;}
        const diff=mainDistance-j.dist;if(diff<-windowDist){status.className='warn';status.textContent='TOO EARLY';detail.textContent='Wait for the spark to reach the orange junction';pinchBtn.classList.add('early');window.setTimeout(()=>pinchBtn.classList.remove('early'),180);sound.beep(260,.04);return;}
      ...
      await new Promise<void>(resolve=>{
        const frame=(now:number)=>{
          ...
          if(phase==='main'){mainDistance+=spec.speed*dt;processPassed();if(mainDistance>=metric.total){mainDistance=metric.total;phase='done';stageResolved=true;for(const j of junctions)if(j.status==='pending')markSkipped(j,'BOMB REACHED');}}
          else if(phase==='branch'&&active){branchPos+=(branchOut?1:-1)*spec.branchSpeed*dt;if(branchOut&&branchPos>=active.branchLen){branchPos=active.branchLen;branchOut=false;sound.beep(920,.045);}else if(!branchOut&&branchPos<=0){branchPos=0;const returned=active;mainDistance=Math.min(metric.total,returned.dist+returned.fuseCost);active=undefined;phase='main';pinchBtn.className='fuse-pinch';pinchBtn.innerHTML='PINCH!<small>Fuse shortened — next junction comes sooner</small>';status.className='';status.textContent=stageIndex===0?'FUSE SHORTENED':'KEEP THE RHYTHM!';detail.textContent=`Next junction arrives ${(returned.fuseCost/spec.speed).toFixed(2)} s sooner because you took the gem`;processPassed();if(mainDistance>=metric.total){phase='done';stageResolved=true;for(const j of junctions)if(j.status==='pending')markSkipped(j,'BOMB REACHED');}}}
          const fusePct=clamp((1-mainDistance/metric.total)*100,0,100);meterFill.style.width=`${fusePct}%`;meterLabel.textContent=`FUSE TO BOMB ${Math.ceil(fusePct)}%`;draw();
          if(stageResolved||phase==='done'){resolve();return;}this.raf=requestAnimationFrame(frame);
        };this.raf=requestAnimationFrame(frame);
      });
      if(this.raf)cancelAnimationFrame(this.raf);if(this.destroyed)return;onPinch=()=>{};pinchBtn.disabled=true;meterFill.style.width='0%';meterLabel.textContent='BOOM';boom.className='fuse-boom show';const stageGems=junctions.filter(j=>j.status==='collected').length;status.className=stageGems>=4?'good':stageGems>=2?'okay':'bad';status.textContent=stageIndex===specs.length-1?'FINAL BOOM!':'BOOM — RAPID RUN NEXT';detail.textContent=`${stageGems}/5 gems collected on ${spec.name} · ${gemCount()}/10 total`;pinchBtn.className='fuse-pinch boom';pinchBtn.innerHTML=`💥 BOOM!<small>${stageIndex===specs.length-1?'Run complete':'Next fuse loading'}</small>`;sound.beep(115,.18);await sleep(stageIndex===specs.length-1?850:1000);
    }
    if(this.destroyed)return;for(let i=0;i<rounds.length;i++)if(rounds[i]<0){rounds[i]=0;errors[i]=900;}const total=totalScore(),gems=gemCount(),collectedErrors=errors.filter((_,i)=>rounds[i]>0),avgError=collectedErrors.length?Math.round(collectedErrors.reduce((a,b)=>a+b,0)/collectedErrors.length):0,secondary=Math.min(20000,errors.reduce((a,b)=>a+b,0));
    status.className=total>=850?'good':total>=650?'okay':total>=350?'warn':'bad';status.textContent=total>=850?'FUSE MASTER!':total>=650?'GEM THIEF!':total>=350?'GOOD RUN':'TAKE MORE RISKS';detail.textContent=`${gems}/10 gems stolen · ${gems?`${avgError} ms average pinch error`:'no successful pinches'}`;this.sendResult(total,secondary,`${total} / 1000 pts · ${gems}/10 gems${gems?` · ${avgError} ms avg pinch`:''}`,rounds);
  }
  async runTimeStop(){
